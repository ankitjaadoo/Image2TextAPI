...
from fastapi import FastAPI, Depends, Request, File, UploadFile, HTTPException, Header
...
def verify_auth(authorization = Header(None), settings: Settings = Depends(get_settings)):
    ...
    if authorization is None:
        raise HTTPException(detail="Invalid authorization header", status_code=400)
    label, token = authorization.split()
    if token != settings.app_auth_token:
        raise HTTPException(detail="Invalid authorization header", status_code=401)
